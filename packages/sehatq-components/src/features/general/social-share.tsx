...
import {
  SocialShareDesktop,
  SocialShareDesktopProps,
} from "./social-share-desktop";
import {
  SocialShareMobile,
  SocialShareMobileProps,
} from "./social-share-mobile";

export type SocialShareProps =
  | ({
      isMobile: true;
    } & Omit<SocialShareDesktopProps, "socialIcons" | "onCopy">)
  | ({ isMobile: false } & Omit<
      SocialShareMobileProps,
      "socialIcons" | "onCopy"
    >);
...
export function SocialShare(props: SocialShareProps) {
  const { isMobile, url, title, sizeIcon, ...otherProps } = props;
  ...
  if (isMobile) {
    return <SocialShareMobile {...baseProps} />;
  }

  return <SocialShareDesktop {...baseProps} />;
}
