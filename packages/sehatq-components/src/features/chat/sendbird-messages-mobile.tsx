...
export type SendbirdMessagesMobileProps = {
...
export function SendbirdMessagesMobile(props: SendbirdMessagesMobileProps) {
  const {
    messages,
    isTyping,
    pagination,
  ...
  return (
    <>
      {!atTop && isLoadingScroll === "up" ? (
        <Flex
          position="absolute"
          top={verticalSpace?.top ? "100px" : "62px"}
          width="full"
      ...
      <Virtuoso
        ...
        itemContent={(index, message) => {
          return <SendbirdBubble isMobile {...message} />;
        }}
...
function Footer(props: ComponentProps) {
  ...
  return (
    <>
      {isTyping ? (
        <SendbirdTypingIndicator isMobile />
      ) : isHistory ? (
        <Text
          mx={3}
          py={3}
