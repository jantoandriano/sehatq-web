...
export type TelemedicineHCPSorterDesktopProps = {
    ...
    <>
      {sorterItems.map((sorter) => (
        <MenuItem
          ...
          onClick={() => {
            navigate(
              props.navigateName ?? "TELEMED_HCPS",
              (oldQuery) => ({
                ...oldQuery,
                sort: sorter.value,
                page: "1",
...
export function TelemedicineHCPSorterDesktop(
  props: TelemedicineHCPSorterDesktopProps
) {
    ...
    <Box fontFamily="openSans" fontSize="sm" fontWeight="semibold">
      ...
      <Menu>
        <MenuButton
          ...
          fontSize="sm"
        >
          {selectedName || "Ketersediaan Dokter"}
        </MenuButton>
...
export function TelemedicineHCPSorterDesktopSkeleton() {
  return (
